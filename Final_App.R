...
ui <- navbarPage("World Bank Visualization App",
 ...
 tabPanel("World Bank Data", sidebarLayout(
       sidebarPanel(h4("Trend Plot Controls"),
          ...
          selectInput("yvar", "Variable to Plot (Y-axis):",choices = names(bank_data)[sapply(bank_data, is.numeric)],
            selected = "population"),
          ...
          selectInput("scatter_x", "X-axis Variable:",
            choices = names(bank_data)[sapply(bank_data, is.numeric)], selected = "GDP_current_US"),
          
          selectInput("scatter_y", "Y-axis Variable:",
            choices = names(bank_data)[sapply(bank_data, is.numeric)], selected = "life_expectancy_at_birth"),
          ...
          selectInput("map_year", "Select Year:", choices = sort(unique(bank_data$year)),selected = 2000),
                                
          selectInput("map_var", "Color Map By:", choices = names(bank_data)[sapply(
            bank_data, is.numeric)], selected = "GDP_current_US")),
                   
          mainPanel(tabsetPanel(tabPanel("Table", DTOutput("data_table")),
 ...
 tabPanel("UK Car Accident Data", sidebarLayout(sidebarPanel(h4("Trend Plot Controls"),
      #Add a dropdown to allow selection of x axis. If year is selected allow for a range to be chosen.
      selectInput("trend_type", "Select Trend Type:",choices = c("Year Range", "Day of Week", "Type of Road")),
                                         
      conditionalPanel(condition = "input.trend_type == 'Year Range'", sliderInput("year_range", "Select Year Range:",
      ...
      h4("Heat Map Controls"),
       ...
       selectInput("heat_year", "Select Year:", choices = sort(unique(car_accidents$year)),
         selected = min(car_accidents$year)), selectInput("severity", "Select Severity Level:", 
...
server <- function(input, output, session) {
  ...
  output$trend_plot <- renderPlot({
    ...
    if (input$trend_type == "Year Range") {
      ...
      ggplot(df, aes(x = year, y = total_casualties)) +
        ...
        geom_point() +
        labs(title = "Casualties by Year", x = "Year", y = "Total Casualties") +
        theme_minimal()
  ...
  output$table <- renderDT({
    datatable(
      car_accidents %>%
        select(Date, year, Day_of_Week, Accident_Severity, Number_of_Casualties, Latitude, Longitude),
